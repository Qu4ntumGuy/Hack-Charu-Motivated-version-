    ...
    def setup(self):
        ...
        for i in range(10):            
            ...
            for o in range(10):
                ...
                y_heights = rand
            for j in range(rand):
                wallins = arcade.Sprite(":resources:images/tiles/boxCrate_single.png", SPRITE_SCALING)
                # wall.bottom = 20 * 
                wallins.center_x = (j)* GRID_PIXEL_SIZE 
                if(random.randint(0,5)==0):
                    # y_heights += random.randint(1,3)  
                    rands = random.randint(1,2)
                    
                    for v in range(rands):
                     wallins.center_y = v*GRID_PIXEL_SIZE


                self.static_wall_list.append(wwallinsall)
